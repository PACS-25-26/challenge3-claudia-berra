...
ForcingKind forcing_from_string(const std::string &name)
{
    if (name == "sine")
    {
        return ForcingKind::Sine;
    }
    if (name == "zero")
    {
        return ForcingKind::Zero;
    }
    if (name == "poly" || name == "polynomial")
    {
        return ForcingKind::Polynomial;
    }
...
{
    if (name == "homogeneous" || name == "zero")
    {
        return BoundaryKind::Homogeneous;
    }
    if (name == "exact")
    {
        return BoundaryKind::Exact;
    }
...
std::string to_string(BoundaryKind kind)
{
    switch (kind)
    {
    case BoundaryKind::Homogeneous:
        return "homogeneous";
    case BoundaryKind::Exact:
        return "exact";
    }
...
double boundary_value(BoundaryKind boundary, ForcingKind forcing, double x, double y)
{
    ...
    case BoundaryKind::Homogeneous:
        return 0.0;
    case BoundaryKind::Exact:
        return exact_solution(forcing, x, y);
    }
